...
var pulsesPerRevolution = 2.0 #  Fan puts out two impulses per revolution
var frequencyTimeTable = 60000.0 # 60,000ms for RPM (Revolutions Per Minute) or 1,000ms for RPS (Revolutions Per Second)
...
class Fan : Driver
    var id
    var lastRPM
    var lastSampledTimeFrame
    var lastPulseCounter
    ...
    def init(id)
        ...
        self.id = id
        self.lastSampledTimeFrame = tasmota.millis()
        self.lastPulseCounter = tasmota.cmd("Counter" + str(self.id))['Counter' + str(self.id)];
        self.lastRPM = 0
    ...
    def update_rpm()
        var currentPulseCounter = tasmota.cmd("Counter" + str(self.id))['Counter' + str(self.id)];
        var currentSampledTimeFrame = tasmota.millis()
        var pulseCounter = currentPulseCounter - self.lastPulseCounter
        var sampledTimeFrame = currentSampledTimeFrame - self.lastSampledTimeFrame

        var scale = frequencyTimeTable / sampledTimeFrame / pulsesPerRevolution
        var currentRPM = math.floor((pulseCounter * scale) + 0.5)

        # Uncomment for debugging
        # print("Fan " + str(self.id) + " PC: " + str(pulseCounter) + " (" + str(currentPulseCounter) + ")" " STF: " + str(sampledTimeFrame) + " Scale: " + str(scale)  + " RPM: " + str(currentRPM) )

        self.lastPulseCounter = currentPulseCounter
        self.lastSampledTimeFrame = currentSampledTimeFrame
